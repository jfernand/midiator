...
class HitVolumeControl : WaveformGenerator {
    ...
    private fun getAmplitude(
        timeStamp: Double, activation: WaveformGenerator.Activation, attack: Double,
        decay: Double, stop: Double, sustain: Double,
    ): Double {
        ...
        var amplitude = (1 - exp(-delta / attack)) * (sustain + (1 - sustain) * exp(-delta / decay))
        if (lastDecayStart > lastHitTime) {
            amplitude *= exp(-(timeStamp - lastDecayStart) / stop)
            if (amplitude < 0.05) {
