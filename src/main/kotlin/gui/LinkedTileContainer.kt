...
class LinkedTileContainer : JPanel(), MouseInputListener {
    ...
    override fun mousePressed(e: MouseEvent) {
        if (SwingUtilities.isLeftMouseButton(e)) {
            ...
            val offsetX = (width / 2) - (components.sumOf { it.x + it.width / 2 } / components.size.toDouble()).toInt()
            val offsetY =
                (height / 2) - (components.sumOf { it.y + it.height / 2 } / components.size.toDouble()).toInt()
    ...
    private fun paintLink(x1: Int, y1: Int, x2: Int, y2: Int, g2d: Graphics2D) {
        ...
        if (Math.abs(dX) > 1) {
            val a = -2 * dY / dX.toDouble().pow(3.0)
            val b = 3 * dY / dX.toDouble().pow(2.0)
            val f = { it: Int -> (a * it.toDouble().pow(3.0) + b * it.toDouble().pow(2.0)).toInt() }
            val draw = {
